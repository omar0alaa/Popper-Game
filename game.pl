% ============================================
% Color Matching Game - New Logic Matching Popper Setup
% ============================================
...
% Initial state: no objects placed
initial_state([]).

% Game loop: ask for moves until all objects are placed
game_loop(State) :-
    (all_objects_placed(State) ->
        write('Game over! All objects placed.'), nl,
        display_state(State)
    ;
        display_state(State),
        write('Enter object (e.g., o1): '), read(Object),
        write('Enter X: '), read(X),
        write('Enter Y: '), read(Y),
        (valid_place_action(Object, X, Y, State) ->
            NewState = [placed(Object, X, Y)|State],
            game_loop(NewState)
        ;
            write('Invalid move! Try again.'), nl,
            game_loop(State)
...
all_objects_placed(State) :-
    ...
    findall(O, member(placed(O, _, _), State), Placed),
    sort(Placed, PlacedSorted),
    sort(Objects, ObjectsSorted),
    PlacedSorted == ObjectsSorted.

% Display the current state
display_state(State) :-
    write('Current placements:'), nl,
    forall(member(placed(O, X, Y), State),
           format('~w at (~w,~w)~n', [O, X, Y])).